...

{
    ...
    //玉発射用
    public GameObject bullet;        // 弾のオブジェクトを用意します
...
//// 物理計算で動かすメソッド
        ...
        //玉発射用
                GameObject bull = Instantiate(bullet);  // prefabのbulletオブジェクトを複製して、変数bullにいれます
                                                        // Playerオブジェクトの子についている1番目のオブジェクトのpositionを入れます
                bull.transform.position = transform.GetChild(1).position;
                Rigidbody bullbody = bull.GetComponent<Rigidbody>();    // Bulletについているrigidbodyを変数に入れます
                                                                        // 方向ベクトル方向に衝撃力を加えます
                bullbody.AddForce(new Vector3(-forceDir.x * pushPower, 0f, -forceDir.z * pushPower), ForceMode.Impulse);
                Destroy(bull, 3.0f); 	// 3秒後に表示を消します
            }
        }
    }
}
