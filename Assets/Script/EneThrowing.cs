...
public class EneThrowing : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //Slerp関数でゆっくり、向かう方向まで回頭をさせます（始めの角度、向かう位置までの角度、時間）　　player.transform.rotation = Quaternion.Slerp(startRot, Quaternion.LookRotation(-forceDir), Time.deltaTime * 5.0f);
    ...
    private void ThrowingBurret()
    {
        if (throwingObj != null && targetPos != null) 	//投げるObjectと狙う場所の値がある場合は・・
        {
            ...
            float angle = throwingAngle;            // 入力された射出角度を変数angleに入れます //ものに当たるように射出する強さ（射出速度）を計算するCaluculateVelocity（座標、目標座標、角度）で算出するスクリプトです
