...
public class CMoveDir : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        player = GameObject.Find("Player-01");           // "Player"という名前のオブジェクトを探して入れます
    }
    ...
    void FixedUpdate()
    {
        ...
        if (Physics.Raycast(ray, out hit))                   //　もしレイが何かに当たったら・・
        {
            ...
            if (Input.GetMouseButton(0))  	　　//　もしMouseボタンが（MouseBotton）押され続けてたら・・

            {
                // 方向ベクトル方向（x,0,z）に衝撃力を加えます・・高さのｙ軸方向は０にします
                player.GetComponent<Rigidbody>().AddForce(new Vector3(-forceDir.x * pushPower, 0f, -forceDir.z * pushPower), ForceMode.Impulse);



                ////ClickVisibleメソッドを呼びます
                ClickVisible();
            }
        }
    }
