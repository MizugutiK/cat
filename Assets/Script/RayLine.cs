...
{
    public int rayBounce;                       　   //Rayが壁に反射する回数を入れます
    ...
    void Start()
    {
        ...
        rayOn = gameObject.GetComponent<CShotDir>().raylineOn;                 //rayを表示するフラグをCSHotDirから取ってきます
    ...
    void Update()
    {
        ...
        if (rayOn == true)				　　　　　 //　もしrayOnフラグがtrueならば・・
        {
            ...
            for (int i = 0; i < rayBounce; i++)                                      　　　　  　  //　変数rayBounceに入れ垂れた回数だけループさせます
            {
                ...
                if (Physics.Raycast(ray.origin, ray.direction, out hit, leftLength))
                {
                    lineRenderer.positionCount += 1;                      　      　　　　//　lineRendererのpositionCountに1を足します
                    {
                        ...
                        if (hit.collider.tag != "Wall") 	 //　もし当たったcolliderのtagが”Wall”　でなければ　(反射させたい壁にWallというtagをセットします)
                        {
                            break;                                  //終わります
                        }
                        else            　　　　　　//もしくは・・　　　　
